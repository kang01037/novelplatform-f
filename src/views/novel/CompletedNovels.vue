  ...
  <div class="completed-page">
    <header class="page-header">
      <div class="header-content">
        ...
        <p class="subtitle">精品完结小说，一次看过瘾</p>
      </div>
        ...
        <div v-for="novel in novels" :key="novel.novelId" class="novel-card">
          <router-link :to="`/novel/detail/${novel.novelId}`" class="card-link">
            <div class="card-cover">
              ...
              <div class="status-badge status-1">已完结</div>
...
const getNovels = async () => {
  try {
    ...
    if (response.data.code === 200 || response.data.message === 'success') {
      ...
      // 只保留已完结
      novelData = novelData.filter(novel => novel.novelStatus === 1)
...
.header-content { margin-bottom: 2rem; text-align: center; }
...
.select-wrapper select option { background: #1a2a3a; color: rgba(255, 255, 255, 0.85); }
.select-wrapper select:hover { border-color: #4facfe; }
.select-wrapper select:focus { outline: none; border-color: #4facfe; box-shadow: 0 0 0 3px rgba(79, 172, 254, 0.15); }
...
.search-icon {
  width: 20px; height: 20px; margin-left: 1rem; color: rgba(168, 216, 234, 0.5);
}

.search-box input {
  border: none; background: transparent; padding: 0.5rem 1rem; font-size: 0.95rem;
  width: 220px; color: rgba(255, 255, 255, 0.85);
}
